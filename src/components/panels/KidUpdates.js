    ...
    <div>
      {!!updates && updates.map( (update, index) => {
        ...
        if (days == 0)
          ...
          lastUpdate = `${days} days ago`;
        else if (weeks >= 1 && weeks <= 10)
          lastUpdate = `${weeks} weeks ago`;
        else if (months > 0 && months <= 12)
          lastUpdate = `${months} months ago`;
        else if( years > 0)
          lastUpdate = `${years} years ago`
        return <div className='update' id={index} style={{overflow:'auto'}}>
          <p className='update-date'>{lastUpdate}</p>
          <Image rounded size={'small'} floated={'left'} src={update.files && "/api/v1/images/" + update.files[0]} alt={index}/>
          <p>{update.body}</p>
          <Divider/>
